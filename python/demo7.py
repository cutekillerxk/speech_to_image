...
async def vad_upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        thread.start()
        print("🚀 已启动后台处理线程，立即返回响应")
        
        # 立即返回响应，不等待处理完成
        global current_record_id
        return {
            "status": "ok",
            "record_id": current_record_id,  # 返回旧的 record_id，前端通过轮询检测新图片
...
async def get_latest_image():
    ...
    try:
        ...
        if history:
            ...
            # 从文件读取图片（确保获取最新图片）
            try:
                image = Image.open(last_record['image_path'])
                ...
                print(f"❌ 读取图片文件失败: {e}")
                return {"status": "error", "msg": f"读取图片失败: {str(e)}"}
        else:
...
() => {
  ...
  // ✅ 生成状态锁：防止并发上传
  window.vadState.isGenerating = false;
  window.vadState.currentCheckIntervalId = null;
  ...
  window.progressUI = {
    ...
    start: function () {
      console.log('[Progress] start() 被调用');
      if (this.isActive) {
        console.log('[Progress] 已在运行，跳过');
        return;
      }
      ...
      console.log('[Progress] 创建 overlay 元素');

      var innerDiv = document.createElement('div');
      innerDiv.style.position = 'relative';
      innerDiv.style.width = '160px';
      ...
      // 确保 body 存在
      if (!document.body) {
        console.error('[Progress] document.body 不存在！');
        return;
      }
      
      document.body.appendChild(overlay);
      console.log('[Progress] overlay 已添加到 body');
      
      // 验证元素是否真的在 DOM 中
      setTimeout(function() {
        var checkOverlay = document.querySelector('.progress-overlay.active');
        console.log('[Progress] 验证 overlay 是否在 DOM 中:', !!checkOverlay);
        if (checkOverlay) {
          var computedStyle = window.getComputedStyle(checkOverlay);
          console.log('[Progress] overlay 样式:', computedStyle.display);
          console.log('[Progress] overlay z-index:', computedStyle.zIndex);
          console.log('[Progress] overlay position:', computedStyle.position);
          // 如果 z-index 还是 auto，强制设置
          if (computedStyle.zIndex === 'auto' || parseInt(computedStyle.zIndex) < 99999) {
            checkOverlay.style.zIndex = '99999';
            console.log('[Progress] 强制设置 z-index 为 99999');
          }
        }
      }, 100);
      ...
      console.log('[Progress] 进度条已启动，overlay:', !!this.overlay, 'circle:', !!this.circle, 'text:', !!this.text);
    ...
    tick: function () {
      ...
      var ratio = Math.min(elapsed / this.maxDuration, 0.99);
    ...
    complete: function () {
      if (!this.isActive || !this.circle) return;

      this.circle.style.strokeDashoffset = 0;
      if (this.text) {
        this.text.innerText = '100%';
      }
  ...
  window.vadStartListening = function () {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      ...
      window.vadState.recorder.onstop = function () {
        // ✅ 检查是否正在生成，如果是则忽略本次录音
        if (window.vadState.isGenerating) {
          console.log('[VAD] ⚠️ 图片正在生成中，忽略本次录音');
          window.vadState.chunks = [];  // 清空录音数据
          return;
        }
        ...
        if (window.vadState.chunks.length === 0) {
          ...
          // ✅ 如果未录到音频，停止进度条
          if (window.progressUI && window.progressUI.isActive) {
            window.progressUI.stop();
          }
          return;
        }
        ...
        fetch('/vad_upload', { method: 'POST', body: formData })
          ...
          .then(function (data) {
            ...
            // ✅ 上传成功后，设置生成锁
            window.vadState.isGenerating = true;
            console.log('[VAD] 🔒 设置生成锁，阻止新上传');
            
            // ✅ 取消之前的轮询（如果存在）
            if (window.vadState.currentCheckIntervalId) {
              clearInterval(window.vadState.currentCheckIntervalId);
              console.log('[VAD] 取消之前的轮询检查');
            }
            ...
            if (window.progressUI) {
              console.log('[VAD] 上传成功，启动进度条');
              window.progressUI.start();
            }
            // 开始轮询检查新图片
            if (window.checkForNewImage) {
              console.log('[VAD] 开始检查新图片，上次ID:', lastRecordId);
              // ✅ 保存当前轮询ID
              var checkIntervalId = window.checkForNewImage(uploadStartTime, lastRecordId);
              window.vadState.currentCheckIntervalId = checkIntervalId;
            } else {
      ...
      fetch('/get_latest_image')
        ...
        .then(function (data) {
          ...
          if (checkCount >= maxChecks) {
            ...
            window.vadState.isGenerating = false;
            window.vadState.currentCheckIntervalId = null;
            console.log('[VAD] 🔓 轮询超时，释放生成锁');
            // ✅ 超时后进度条保持在 99%，不隐藏（继续等待）
            // 如果需要隐藏，可以调用：window.progressUI.stop();
          }
        })
        .catch(function (error) {
  ...
  window.autoFullscreenImage = function () {
    ...
    // 监听图片更新事件（Gradio 更新图片时）- 使用防抖减少执行频率
    var fullscreenTimeout = null;
    var observer = new MutationObserver(function (mutations) {
      // 防抖：只在最后一次变化后 500ms 执行
      if (fullscreenTimeout) {
        clearTimeout(fullscreenTimeout);
      }
      fullscreenTimeout = setTimeout(tryFullscreen, 500);
    });
